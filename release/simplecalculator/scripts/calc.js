...
function sendResult(expression) {
  try {
    ...
    ChatMessage.create({
      content: `<strong>Resultado:</strong> ${foundry.utils.escapeHTML(expression)} = <strong>${formatResult(result)}</strong>`
    });
  } catch (error) {
    ChatMessage.create({
      content: `<strong>Calculadora:</strong> ${foundry.utils.escapeHTML(error.message)}`
    });
  }
}
